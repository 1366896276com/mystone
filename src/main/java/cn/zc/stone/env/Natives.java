...
import cn.zc.stone.ast.StringLiteral;
...
public class Natives {
    ...
    private void appendNatives(Environment env) {
    ...
    private void append(Environment env, String name, Class<?> clazz, String mathodName, Class<?>... params) {
        Method m;
        try{
            m=clazz.getMethod(mathodName,params);
        }catch (Exception e){
            throw new StoneException("cannot find a native function: "+mathodName);
        }
        env.put(name,new NativeFunction(mathodName,m));
    }
